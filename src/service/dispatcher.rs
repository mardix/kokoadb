...
async fn dispatch_inner(
    ...
    let mut result = match operation.as_str() {
        ...
        "file_create" => heap_future(file_create(state, db_path, &conn, req)).await,
        "file_get" => heap_future(file_get(&conn, req)).await,
        "file_query" => heap_future(file_query(&conn, req)).await,
        "file_update" => heap_future(file_update(state, db_path, &conn, req)).await,
        "file_delete" => heap_future(file_delete(state, db_path, &conn, req)).await,
