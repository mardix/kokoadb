...
pub async fn meta_operations(
  ...
  "operations":{{
    ...
    "purge_system_db":{{"writes":true,"required":[],"optional":[],"description":"Delete the local __kdb_system.db catalog and immediately rebuild it from currently discoverable databases"}},
    ...
    "get_data_count":{{"writes":false,"required":[],"optional":[],"description":"Return exact current counts for documents with namespace breakdowns, identity statuses, non-deleted files, metric events, and all user-created SQL tables"}},
    ...
    "delete_db":{{"writes":true,"required":[],"optional":[],"description":"Synchronously snapshot and archive-backup the current database, then permanently remove its local and live S3 artifacts"}},
    ...
    "user_get":{{"writes":false,"required":["user_id|id|email|username|provider+provider_user_id"],"optional":["include_credentials(default=false)"],"description":"Fetch one identity user by local id, email, username, or provider mapping. include_credentials=true adds password_hash and password_algo to the returned item"}},
    ...
    "file_query":{{"writes":false,"required":[],"optional":["bucket","status","owner_type","owner_id","storage_backend","content_type","search|q","filter(Filter Operators on metadata.*)","limit","offset","page","per_page"],"description":"List/query visible non-deleted file metadata rows with pagination and JSON metadata filtering"}},
