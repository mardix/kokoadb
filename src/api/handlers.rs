...
pub async fn meta_operations(
  ...
  "operations":{{
    ...
    "user_get_credentials":{{"writes":false,"required":["user_id|id|email|username|provider+provider_user_id"],"optional":[],"description":"Fetch password verification material for one identity user. Returns password_hash only through this explicit uncached operation"}},
